...
const BUILDER_TABLE_NAMES = [
  "product_page_configs",
  "home_page_configs",
] as const;

const TABLE_NAMES = [...CORE_TABLE_NAMES, ...BUILDER_TABLE_NAMES] as const;
...
export async function getSupabaseHealth(): Promise<SupabaseHealth> {
  ...
  try {
    ...
    const connected =
      tables.settings && tables.products && tables.orders && tables.admins;
    const builderTablesReady =
      tables.product_page_configs && tables.home_page_configs;
    ...
    if (connected) {
      const [products, orders, reviews, admins, storage] = await Promise.all([
